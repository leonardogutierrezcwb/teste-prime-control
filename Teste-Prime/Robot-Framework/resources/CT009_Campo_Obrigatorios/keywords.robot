...
Preencher E-mail
    ...
    Input Text    ${XPATH_EMAIL}    ${EMAIL_INVALIDO}
...
VerificarEAdicionarBotaoSalvar
    ...
    Wait Until Element Is Visible    ${XPATH_SALVAR}    timeout=10s  # Aguarda o botão ser visível
...
Dados Inválidos
    [Documentation]    Insere dados fakes.
    Input Text    ${XPATH_NOME_COMPLETO}    0
    Input Text    ${XPATH_TELEFONE}    0
    Input Text    ${XPATH_EMAIL}    0@teste.fakes
    Input Text    ${XPATH_CEP}    99999-999
    Input Text    ${XPATH_NUMERO_RESIDENCIA}    0
    Input Text    ${XPATH_ENDERECO}    0
    Input Text    ${XPATH_COMPLEMENTO}    0
    Click Element    ${XPATH_PAIS}
    Wait Until Element Is Visible      ${XPATH_PAIS_BRASIL}
    Click Element    ${XPATH_PAIS_BRASIL}
    Click Element    ${XPATH_GENERO}
    Click Element    ${XPATH_GENERO_OUTROS}
    Execute Javascript    window.scrollBy(0, 700);  # Rola 500px para baixo
    Execute Javascript    window.scrollBy(0, 700);  # Rola 500px para baixo  
    Wait Until Element Is Visible    ${XPATH_BOTAO_SALVAR}
##    Clicar em Salvar
